...
public class PinpadTestActivityForVersion1 extends BaseActivity {

    private static final String TAG = PinpadTestActivityForVersion1.class.getName();
    ...
    protected View onCreateView(LayoutInflater inflater) {
        return inflater.inflate(R.layout.activity_pinpad_test_version1, null);
    }
    ...
    public void loadProtectKey(View view) {
        ...
        try {
            boolean iRet = ServiceManager.getInstence().getPinpad().loadProtectKey(protect);
            if (iRet) {
                showToast("load protect Key Success!"+" protect key = "+protect);
                GlobalData.getInstance().setPinpadVersion(PinpadInterfaceVersion.PINPAD_INTERFACE_VERSION1);
            } else {
    ...
    public void loadMainKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(mainKcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadMainKey(main);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadMainKeyWithKcv(main, mainKcv);
            }
            if (iRet) {
    ...
    public void loadMacKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(macKcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadMacKey(mac, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadMacKey(mac, macKcv);
            }
    ...
    public void loadPinKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(pin_kcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadPinKey(pin, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadPinKey(pin, pin_kcv);
            }
    ...
    public void loadTDKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(td_kcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadTDKey(tdkey, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadTDKey(tdkey, td_kcv);
            }
...
//        ServiceManager.getInstence().getPinpad().calcMAC()
        ...
        try {
            String mac = ServiceManager.getInstence().getPinpad().calcMAC(demoData, BwPinpadSource.MAC_ECB);
            setHint("calMAC:");
    ...
    public void encryptedTrack(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().encryptMagTrack(demoData);
    ...
    public void inputonlinepin(View view) {
        ...
        PWDialog pwDialog = new PWDialog(this,PinpadInterfaceVersion.PINPAD_INTERFACE_VERSION1,0,0);
    ...
    public void encryptedData(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().encryptData(type, demoData);
    ...
    public void decryptData(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().decryptData(type, encryptedData);
            setHint("decryptData:");
    ...
    private void initView() {
        ...
        sp1.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                switch (position) {
                    case 0:
                        PinpadTestActivityForVersion1.this.type = KeyType.PIN_KEY;
                        break;
                    case 1:
                        PinpadTestActivityForVersion1.this.type = KeyType.TRACK_KEY;
                        break;
                    case 2:
                        PinpadTestActivityForVersion1.this.type = KeyType.MAC_KEY;
                        break;
                }
            }
