...
public class PinpadTestActivityForVersion2 extends BaseActivity {

    private static final String TAG = PinpadTestActivityForVersion2.class.getName();
    ...
    protected View onCreateView(LayoutInflater inflater) {
        return inflater.inflate(R.layout.activity_pinpad_test_version2, null);
    }
    ...
    public void loadProtectKey(View view) {
        ...
        try {
            boolean iRet = ServiceManager.getInstence().getPinpad().loadProtectKeyNew(protect);
            if (iRet) {
                showToast("load protect Key Success!area = "+" protect key = "+protect);
                GlobalData.getInstance().setPinpadVersion(PinpadInterfaceVersion.PINPAD_INTERFACE_VERSION2);
            } else {
    ...
    public void loadMainKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(mainKcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadMainKeyNew(tmkindex,main);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadMainKeyWithKcvNew(tmkindex,main, mainKcv);
            }
            if (iRet) {
    ...
    public void loadMacKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(macKcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadMacKeyNew(tmkindex,mac, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadMacKeyNew(tmkindex,mac, macKcv);
            }
    ...
    public void loadPinKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(pin_kcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadPinKeyNew(tmkindex,pin, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadPinKeyNew(tmkindex,pin, pin_kcv);
            }
    ...
    public void loadTDKey(View view) {
        try {
            ...
            if (TextUtils.isEmpty(td_kcv)) {
                iRet = ServiceManager.getInstence().getPinpad().loadTDKeyNew(tmkindex,tdkey, null);
            } else {
                iRet = ServiceManager.getInstence().getPinpad().loadTDKeyNew(tmkindex,tdkey, td_kcv);
            }
...
//        ServiceManager.getInstence().getPinpad().calcMAC()
        ...
        try {
            String mac = ServiceManager.getInstence().getPinpad().calcMACNew(tmkindex,demoData, BwPinpadSource.MAC_ECB);
            setHint("calMAC:");
    ...
    public void encryptedTrack(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().encryptMagTrackNew(tmkindex, demoData);
    ...
    public void inputonlinepin(View view) {
        ...
        PWDialog pwDialog = new PWDialog(this,PinpadInterfaceVersion.PINPAD_INTERFACE_VERSION3,0,tmkindex);
    ...
    public void encryptedData(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().encryptDataNew(tmkindex, type, demoData);
    ...
    public void decryptData(View view) {
        ...
        try {
            String s = ServiceManager.getInstence().getPinpad().decryptDataNew(tmkindex, type, encryptedData);
            setHint("decryptData:");
    ...
    private void initView() {
        ...
        sp1.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                switch (position) {
                    case 0:
                        PinpadTestActivityForVersion2.this.type = KeyType.PIN_KEY;
                        break;
                    case 1:
                        PinpadTestActivityForVersion2.this.type = KeyType.TRACK_KEY;
                        break;
                    case 2:
                        PinpadTestActivityForVersion2.this.type = KeyType.MAC_KEY;
                        break;
                }
            }
...
//        Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
    ...
    {
        Intent intent = new Intent(this, PinpadTestActivityForVersion1.class);
        startActivity(intent);
    }

    public void enterPinpadTestVersion2(View view)
    {
        Intent intent = new Intent(this, PinpadTestActivityForVersion2.class);
        startActivity(intent);
    }
